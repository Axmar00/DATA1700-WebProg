package com.asmar.webprog.inputvalidering.repository;
...
@Repository
public class MotorvognRepository5 {
    @Autowired
    private JdbcTemplate db;

    private Logger logger = LoggerFactory.getLogger(MotorvognRepository5.class);

    public boolean lagreMotorvogn(Motorvogn innMotorvogn) {
        String query = "INSERT INTO Motorvogn5 (personNr,navn,adresse,kjennetegn,merke,type) VALUES(?,?,?,?,?,?)";
        try {
    ...
    public List<Motorvogn> hentAlleMotorvogner() {
        String query = "SELECT * FROM Motorvogn5";
        try {
    ...
    public boolean slettAlleMotorvogner() {
        String query = "DELETE FROM Motorvogn5";
        try {
            db.update(query);
            return true;
        } catch(Exception e) {
    ...
    public boolean slettEnMotorvogn(int id) {
        String query = "DELETE FROM Motorvogn5 WHERE id=?";
        try {
            db.update(query, id);
            return true;
        } catch(Exception e) {
    ...
    public Motorvogn hentEnMotorvogn(int id) {
        ...
        param[0] = id;
        String query = "SELECT * FROM Motorvogn5 WHERE id=?";
        try {
    ...
    public boolean endreMotorvogn(Motorvogn m) {
        String query = "UPDATE Motorvogn5 SET personNr=?,navn=?,adresse=?,kjennetegn=?,merke=?,type=? where id=?";
        try {

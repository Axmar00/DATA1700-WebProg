...
@Repository
public class MotorvognRepository6 {
    @Autowired
    private JdbcTemplate db;

    private Logger logger = LoggerFactory.getLogger(MotorvognRepository6.class);

    public boolean lagreMotorvogn(Motorvogn innMotorvogn) {
        String query = "INSERT INTO Motorvogn6 (personNr,navn,adresse,kjennetegn,merke,type) VALUES(?,?,?,?,?,?)";
        try {
    ...
    public List<Motorvogn> hentAlleMotorvogner() {
        String query = "SELECT * FROM Motorvogn6";
        try {
    ...
    public boolean slettAlleMotorvogner() {
        String query = "DELETE FROM Motorvogn6";
        try {
            db.update(query);
            return true;
        } catch(Exception e) {
    ...
    public boolean slettEnMotorvogn(int id) {
        String query = "DELETE FROM Motorvogn6 WHERE id=?";
        try {
            db.update(query, id);
            return true;
        } catch(Exception e) {
    ...
    public Motorvogn hentEnMotorvogn(int id) {
        ...
        param[0] = id;
        String query = "SELECT * FROM Motorvogn6 WHERE id=?";
        try {
    ...
    public boolean endreMotorvogn(Motorvogn m) {
        String query = "UPDATE Motorvogn6 SET personNr=?,navn=?,adresse=?,kjennetegn=?,merke=?,type=? where id=?";
        try {
